...
const fileRef = (file: File, index: number): AttachmentRef => ({
  id: `${file.name}-${file.size}-${file.lastModified}-${index}`,
  displayName: file.name,
  mimeType: file.type || "application/octet-stream",
  size: file.size,
});
const attachmentVisual = (attachment: AttachmentRef) =>
  attachment.mimeType === "application/x-directory"
...
export function CreateProjectDialog({ onClose, onCreate }: Props) {
  ...
  const selectFolder = (displayName: string) => {
    ...
    setAttachments((current) => [
      attachment,
      ...current.filter((item) => item.mimeType !== "application/x-directory"),
    ]);
    setInitialMessage("");
  };
  ...
  return (
    ...
    >
      <section className="create-project-card">
        <header className="create-project-head">
          <h2>创建项目</h2>
        </header>
        <button
        ...
        <div className="project-identity">
          <div className={`project-avatar ${avatar ? "has-avatar" : ""}`}>
            <button
              className="project-avatar-picker"
              type="button"
              aria-label="选择项目头像"
              title="选择项目头像"
            ...
            >
              {avatar ? (
                <img src={avatar.dataUrl} alt="项目头像" />
              ) : (
                <img className="project-avatar-plus" src={plusIcon} alt="" />
              )}
            </button>
            {avatar && (
              <button
                className="project-avatar-remove"
                type="button"
                aria-label="删除项目头像"
          ...
          <div className="project-name-shell">
            ...
            >
              <img className="project-folder-icon" src={folderIcon} alt="" />
            </button>
            <i />
            <input
              aria-label="项目名称"
        ...
        <div className="project-context">
          <strong>需求描述</strong>
          <span className={workingFolder ? "selected" : ""} data-testid="working-folder-status">
            {workingFolder ? `已添加源文件 · ${workingFolder.displayName}` : "未添加源文件"}
          </span>
        </div>
        <div
          className={`project-composer ${dragging ? "drag" : ""}`}
          onDragEnter={(event) => {
            event.preventDefault();
            setDragging(true);
          }}
        ...
        >
            ...
            {attachments.map((attachment) => {
              ...
              return (
                <div
                  ...
                  data-testid={`attachment-${attachment.displayName}`}
                  data-attachment-kind={visual.kind}
                  key={attachment.id}
                >
          ...
          <textarea
            className={attachments.length ? "has-attachments" : ""}
            aria-label="项目需求"
            value={initialMessage}
            onChange={(event) => setInitialMessage(event.target.value)}
            placeholder={attachments.length ? "" : placeholder}
            autoComplete="off"
            spellCheck={false}
          />
        </div>
        <footer className="create-project-footer">
          <div className="project-options">
            ...
            <div className="project-option industry">
              <span>行业数据</span>
              <Switch
                checked={industryIntelligenceEnabled}
                label="切换行业数据"
                onChange={setIndustryIntelligenceEnabled}
              />
              <span className="project-help" tabIndex={0} aria-label="行业数据帮助">
                !<span>启用后可获得最新行业数据，CL 消耗会相应增加。</span>
              </span>
            </div>
          </div>
        ...
        <input
          ...
          type="file"
          accept="image/*"
          onChange={(event) => chooseAvatar(event.target.files)}
        />
      </section>
    </dialog>
  );
...
function Option({
  ...
  return (
    <div className="project-option">
      <span>{label}</span>
      <Switch checked={checked} label={`切换${label}`} onChange={onChange} />
    </div>
  );
}
...
function Switch({
  ...
  return (
    <button
      className={`project-switch ${checked ? "on" : ""}`}
      type="button"
      aria-label={label}
