...
describe('CreateProjectDialog', () => {
  it('closes from close and cancel controls', () => {
    const onClose = vi.fn();
    const { rerender } = render(<CreateProjectDialog onClose={onClose} onCreate={vi.fn().mockResolvedValue(undefined)} />);
    fireEvent.click(screen.getByRole('button', { name: '关闭创建项目' }));
    expect(onClose).toHaveBeenCalledTimes(1);
    rerender(<CreateProjectDialog onClose={onClose} onCreate={vi.fn().mockResolvedValue(undefined)} />);
    fireEvent.click(screen.getByRole('button', { name: '取消' }));
    expect(onClose).toHaveBeenCalledTimes(2);
  });

  it('keeps v0.32 avatar, switch, folder, and attachment state', async () => {
    render(<CreateProjectDialog onClose={vi.fn()} onCreate={vi.fn().mockResolvedValue(undefined)} />);
    ...
    expect(screen.getByRole('button', { name: '切换模板配置' })).toHaveAttribute('aria-pressed', 'true');
    expect(screen.getByRole('button', { name: '切换行业数据' })).toHaveAttribute('aria-pressed', 'false');
    fireEvent.change(screen.getByLabelText('头像输入'), { target: { files: [new File(['avatar'], 'avatar.png', { type: 'image/png' })] } });
    ...
    expect(document.querySelector('.project-avatar-plus')).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText('工作文件夹输入'), { target: { files: [new File(['x'], 'DemoFolder', { type: 'text/plain' })] } });
    expect(screen.getByTestId('working-folder-status')).toHaveTextContent('DemoFolder');
    expect(screen.getByTestId('attachment-DemoFolder')).toHaveAttribute('data-attachment-kind', 'folder');
    ...
    expect(screen.getByLabelText('项目名称')).toHaveValue('发布项目');
    fireEvent.drop(screen.getByLabelText('项目需求').parentElement!, { dataTransfer: { items: [], files: [new File(['x'], 'brief.md', { type: 'text/markdown' })] } });
    expect(screen.getByRole('button', { name: '移除附件 brief.md' })).toBeInTheDocument();
    expect(screen.getByTestId('attachment-brief.md')).toHaveAttribute('data-attachment-kind', 'file');
  });
  ...
  it('shows drag feedback and distinguishes image attachments', () => {
    ...
    fireEvent.drop(composer, { dataTransfer: { items: [], files: [new File(['image'], 'cover.png', { type: 'image/png' })] } });
    expect(screen.getByTestId('attachment-cover.png')).toHaveAttribute('data-attachment-kind', 'image');
  });
  ...
  it('submits the unified project payload', async () => {
    ...
    await waitFor(() => expect(onCreate).toHaveBeenCalledWith(expect.objectContaining({ name: '项目 Agent', workingFolder: null, initialMessage: '执行项目目标', templateEnabled: true, industryIntelligenceEnabled: true })));
  ...
  it('uses an uploaded avatar as the Project.avatar value', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '创建项目' }));
    await waitFor(() => expect(onCreate).toHaveBeenCalledWith(expect.objectContaining({ avatar: expect.objectContaining({ source: 'upload', name: 'avatar.png', dataUrl: expect.stringContaining('data:image/png;base64,') }) })));
  });
});
