...
export function buildSystemPrompt() {
  return [
    ...
    '',
    '共识记录规则(重要,决定哪些内容进入项目共识待确认区):',
    '- 本轮如有值得记入共识的具体建议(限5条内,每条40字内),在回复最后另起一段写:【共识建议】换行逐条,最后以【共识建议结束】收尾。',
    '- 本轮如有需要用户拍板的关键问题(限5个内),同样包裹在【待确认】与【待确认结束】之间,每行一个。',
    '- 没有就不写,不要硬凑;正文里不要复述这些标记,它们会被自动收录并不显示给用户。',
    '',
    guideText(),
  ].join('\n');
}
