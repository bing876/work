...
export function buildSystemPrompt(memoryDigest = '', pending = '') {
  const memory = memoryDigest ? [`【你目前已记住(供判断新旧冲突用,不要复述给用户)】`, memoryDigest, ''] : [];
  const todos = pending ? [`【待确认方案(后台编号,只用来写动作块,不要告诉用户编号)】`, pending, ''] : [];
  return [...memory, ...todos,
    ...
    '',
    '任务动作协议(后台能力,动作块会被执行并不显示,绝不在正文里提标记和编号):',
    '用户请你安排某件事、你给出具体方案时,在回复最后用【提议】块写一遍:标题：..(20字内);内容：..(100字内);用【提议结束】收尾。',
    '用户明确说确认/好的/开始/就这么办时,用【确认】块写:任务：待确认方案中的编号;版本：该方案版本号;用【确认结束】收尾。用户没明确确认、还在犹豫时,绝不发【确认】。',
    '用户说算了/先不做/取消时,用【取消】块写任务编号;用【取消结束】收尾。',
    '用户说改一下/调整并给出新要求时,用【修订】块写任务编号和新的标题/内容;用【修订结束】收尾。',
    '一次回复只做一个动作;绝不能在同一回复里既【提议】又【确认】,新方案必须等用户亲口确认。',
    '收尾标记必须严格写成【XX结束】,不要写成【/XX】或其他形式。',
    '',
    guideText(),
  ].join('\n');
}
