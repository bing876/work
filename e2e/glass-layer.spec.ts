...
test("uses one frame glass surface with transparent internal regions", async ({ page }) => {
  ...
  for (const selector of [".rail", ".sidebar"]) {
    expect(
      await page.locator(selector).evaluate((element) => getComputedStyle(element).backgroundImage),
  ...
  for (const selector of [".menu-btn", ".tab", ".hamburger-btn"]) {
    ...
    expect(Math.abs(box!.x + box!.width / 2 - (railBox!.x + railBox!.width / 2))).toBeLessThan(
      0.01,
    );
  }
