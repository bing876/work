...
test('creates a project through the injected mock workbench flow', async ({ page }) => {
  const output = '../../../visual-review/workbench-vnext-visual-source-of-truth-v0.1/phase06-experience-simulation';
  ...
  await expect(page.locator('.rail').getByRole('button', { name: '品牌发布' })).toHaveAttribute('aria-pressed', 'true');
  await page.getByRole('button', { name: '退出 Agent Switcher' }).click();
  await expect(page.locator('.sidebar').getByRole('button', { name: /品牌发布/ })).toHaveAttribute('aria-pressed', 'true');
  await expect(page.getByRole('region', { name: '对话' }).getByText('为新品发布准备完整方案')).toBeVisible();
  ...
  await expect(page.getByRole('region', { name: '对话' }).getByText(/已完成模拟执行/)).toBeVisible();
...
test('captures Phase 0.5 create-project visual states', async ({ page }) => {
  const output = '../../../visual-review/workbench-vnext-visual-source-of-truth-v0.1/phase053-sidebar-visual-rebalance';
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto('/');
  ...
  await page.getByLabel('项目需求').evaluate((textarea) => {
    ...
    textarea.parentElement?.dispatchEvent(new DragEvent('drop', { bubbles: true, dataTransfer: transfer }));
  });
  await page.getByLabel('项目需求').fill('为新品发布准备完整方案');
